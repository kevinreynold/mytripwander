  ...
  <f7-page>
    ...
    <f7-tabs animated swipeable>
      <f7-tab id="tab1" active>
        <div v-for="item in modified_run_down" :key="item.key">{{item.string_format}}</div>
...
export default {
  ...
  mounted() {
    ...
    console.log(window.Dom7('.searchbar'));
  },
  created() {
    self = this;
    self.modified_run_down = copy(store.coba_modified_run_down);
  ...
  methods: {
    ...
    searchPlace(mode){ //attraction | food
      ...
      // console.log(mySearchbar.params);
      // mySearchbar.clear();
      setTimeout(function(){
